...
| A. Hráč a zaměření (STŘEDNÍ mimo body) | PLR-1, PLR-2, WLD-7 (+ PLR-3, WLD-11) | ✅ opraveno: creative s drženým tlačítkem rozbije další blok až po 0,25 s (`Mining.CREATIVE_DELAY`, 5 ticků jako vanilla), puštění prodlevu vynuluje, mířením na oblohu se nepřeskočí; `cancel()` (Main) vs. vnitřní `stop()`. Raycaster: nulová složka směru = nekonečno (`firstBoundary`), buňka za `maxDistance` se netestuje. Hráč: sonda země `2·EPSILON` pod nohama, takže `onGround` platí v každém framu i nad 167 FPS. Testy: `CreativeTest` (klik 100 ms = 1 blok při 30/60/240/1000 FPS, držení 1 s = 4 bloky, nový klik, obloha, survival beze změny), `RayTest` (start na celé Y/Z, −0,0, nulový vektor, dosah), `CameraTest` (citlivost, obrácená osa, ořez ±89, konečná matice), `PhysicsTest` při 30–1000 FPS. 2238 kontrol. |
| B. Recepty, atlas, texty labu (STŘEDNÍ mimo body) | INV-3, PER-4, LAB-8 (+ LAB-14) | ✅ opraveno: `Recipes.builtInHasPattern()` položí vzor do mřížky 3×3 a zkusí tvarované i bezetvaré vestavěné recepty (týmž porovnáním jako hra), takže lab „jednu trávu“ ani „prkno + uhlí“ neuloží. `Textures.completeAtlas()` doplní atlas ze souboru i z importu: prázdné vestavěné buňky procedurálně, prázdné buňky bloků z labu šachovnicí (namalované nepřepíše). Biomes: varování „Save = NEXT world, not this one“ má pevné místo vpravo a zkracuje se popis stromu; řádek CLASH v Keys přes `fit()`. Testy: `RecipeLabTest` (4 bezetvaré jednosložkové, prkno + uhlí i rozházené v 3×3, tráva + kámen a dvě trávy ne, lab trávu odmítne), `LabBlockTest` (starý atlas ze souboru, zachovaná namalovaná dlaždice, import = načtení). Sonda s GL: řádek v Biomes i kolize v Keys se vejdou. 2252 kontrol. |
| C. Jeskyně u strmého svahu (STŘEDNÍ mimo body) | GEN-4 | ✅ opraveno: `generateColumn()` počítá výšky i o sloupeček za okrajem chunku a kope jen pod nejnižším ze čtyř sousedů (`caveCeiling`), takže jeskyně nesousedí s vodou ani vzduchem nižšího sloupce. Výchozí tuning má krok nejvýš 3 (změřeno na 3000×3000 sloupcích), mez u něj nikdy nezabere: `SeedTest` beze změny, `GENERATOR_VERSION` zůstává 5. Natuněné světy se změní jen v buňkách, kde byla jeskyně otevřená do svahu (tuning se ke světu neukládá, takže to platí i pro už uložené světy). Nový test v `CaveTest` na strmém tuningu: bez meze 57 stěn k vodě a 16 vchodů (ověřeno vypnutím meze), s ní 0. A/B generování sloupce: 1,38–1,48 → 1,28–1,38 ms (v šumu). 2256 kontrol. |
| D. Render (STŘEDNÍ mimo body) | REN-4, REN-5 | ✅ opraveno (rozhodnutí uživatele: „jako svět všude“): průhledný pixel má všude svou barvu, alfu bere jen voda (`World.isTranslucent`, používá ho i mesher). Náhled kůže, bloku a stromu kreslí neprůhledný průchod výslovně bez míchání; ruka má `uKeepAlpha`, ikona příznak ve vrcholu (6. float). Stavba vrcholů `BlockIcon` je statická `build()` bez GL a nový `BlockIconTest` hlídá otočení trojúhelníků, meze, plochu, UV, odstíny a příznak alfy. Sonda s GL: vygumovaný kámen černý v ikoně (120 000 px = 3/4 čtverce), v ruce, holá ruka a náhled kůže černé, voda v ikoně průhledná, žádná chyba GL; hra naběhne. Nezměněno: voda jako položka na zemi se kreslí neprůhledně (položky jdou neprůhledným průchodem). 2296 kontrol. |
| E. `.gitignore` (STŘEDNÍ mimo body) | PER-5 | ✅ opraveno (rozhodnutí uživatele: „vše mimo git“): ignorované jsou `keybinds.json`, `biome_tuning.json`, `textures/atlas.png`, `skin.png`, `blocks.json`, `recipes.json`, `import.png`, `sounds/` a zálohy `*.bak`, `*.bak.*`, `*.tmp`; `atlas.png` a `recipes.json` přestaly být trackované (`git rm --cached`, na disku zůstaly). Ověřeno `git check-ignore`. ⚠️ Kdo repo už má a stáhne tenhle commit, tomu git oba soubory z pracovního adresáře smaže (hra pak jede s procedurálním atlasem a vestavěnými recepty). |
| 12a. Úklid: build, mrtvý kód, komentáře | BLD-1, LAB-13, REN-10, PLR-8, GEN-11, GEN-12, GEN-15, INV-5, PER-13, REN-12, SND-5, zbytek LAB-12; části GEN-13, WLD-12, INV-7, UI-7, UI-8 | ✅ `pom.xml` má `maven.compiler.release` 17 (hlídá i API, build bez varování). Smazaný nepoužívaný kód (`hitPanel`, `panelTextLeft`, `TextureLab.mode/currentModeTitle/modeCount`, `faceColors`, `tileSnapshot`, `clearUndo`, `meshesBuilt`, `FontAtlas.ascent()`, volná kamera v `Camera`, `WindowMode.isFullscreen`, `TerrainGenerator.DEFAULT`, `TextField.maxLength()`, nepoužitý `world` v `Hud`). `ItemStack.sameItem()` je jediné místo pravidla „jde slít“ (5 míst přepsáno). „kmen + 2“ jen v `TreeShape.totalHeight()`. `SAND_LEVEL = World.SEA_LEVEL + 1` a `GROUND_HEIGHT` z `World` (SeedTest beze změny). Klíč sekce v `LightEngine` má 28 bitů na osu (dřív `sectionKey(-524288,0,0) == EMPTY`), test v `LightTest`; `nanoTime` přes rozdíl; strop převzatých sloupců počítá jen převzaté. `OptionsTest.java` už není binární (escape ` `). Opravené zastaralé komentáře (Json, SafeFiles, WorldStorage, BlockAtlas, Texture, FontAtlas, Renderer2D, SkyRenderer, ChunkMesh, osiřelé Javadocy v Shaders a LightEngine, CreateWorldScreen, Hud, Sound, seedMix, IRON_RARITY_MOUNTAINS, MAX_CROWN, TextureLab, LabMode press/key, TextureLabLayout, RecipeBook, Recipe). Vědomě nezměněno: pole vzorů v `Recipe` se nekopírují (zdokumentovaná dohoda), testovací pomocníci `isClamped`/`variesTreeSize`/`TreeType.totalHeight` zůstávají, `getBlock(y<0)` = kámen proti `cellAt(y<0)` = vzduch (každé pro svůj účel). 2298 kontrol. |
| 12b. Úklid: drobné chyby chování | MAIN-13, MAIN-14, INV-8, INV-9, LAB-15, LAB-17, GEN-8, GEN-9, GEN-10, GEN-13, GEN-14, WLD-13, REN-6, REN-7, REN-8, REN-9, PLR-4, PLR-5, PLR-6, SND-4, UI-3, UI-4, UI-6, zbytek UI-7, PER-11, část PER-9, část PLR-7 | ✅ zaměření se po rozbití počítá znovu; worker `World` se spouští líně a Save and Quit starý svět zastaví a uvolní (i meshe a položky); časovače položek skutečným časem; hromádka na kurzoru i s počtem kreslená naposled; tah bez změny nedělá krok undo, hex se při odchodu z módu zahodí, New tile preferuje prázdnou buňku a řekne, když přepisuje; plošinka náhledu stromu je první blok sekce 4 (terén se nemešuje); koruna ne pod patu kmene; `TreeSpot` nese biom a výšku do `placeTree` (SeedTest beze změny); testy dělitelnosti žil, round-trip tuningu, mezí tuningu a spawnu bez souše; testy zastavují své světy; žádné `glLineWidth(3)`; `ShaderProgram` uklidí po chybě a nealokuje matici; buffer ruky podle `BlockModels.MAX_BOXES`; náhledy světů se uvolní při odchodu ze seznamu; `TextRenderer.fit/fitEnd` půlením a bez měřítka; fáze/čas animace a yaw se balí; animace s ořezaným dt; selhaný fullscreen vrátí nastavení; `WAVE_FORMAT_EXTENSIBLE` podle subformátu; F3 s `Locale.ROOT`; pole Create ukazují konec textu; posuvník hlásí změnu jen při jiném kroku; `MenuTest` bez tautologie, test vkládání diakritiky; novější `world.dat` se hlásí jako novější, `DirectoryIteratorException` se chytá; `format` se všude porovnává číselně a `Options` používá `notExists`; kolečko se sčítá (trackpad). 2322 kontrol; sondy s GL (lab, náhled stromu) a smoke run hry. |
| 12c. Úklid: dokumentace a co zůstává | DOC-1, DOC-2, DOC-3; vědomě beze změny: MAIN-15, PER-12, INV-6, REN-11 (jen pojmenované duplicity, audit refaktor nenavrhuje), LAB-11, REN-13, GEN-7, zbytek PLR-7, zbytek PER-9, WLD-12 (fronty světla), INV-7 (kopie polí) | ✅ `ARCHITECTURE.md`: dlaždice (vestavěné končí na 34, volných 29, varování o buňkách 27–34 u starých bloků z labu), 84 bloků v přehledu, skok 1,33 při 60 FPS (a proč se liší od 1,26), klesání ve vodě ~1,8 b/s, vertex meshe 7 floatů, poškozený soubor svět nenačte (nezaloží nový), novější formát se hlásí jako novější, async generování hotové (frustum culling ne), paměť meshů jen na GPU, značení až 8 sekcí po změně, `Load World` → Select World, chybějící řádky `SafeFilesTest` a `PerfTest`. **Beze změny a proč:** duplicity MAIN-15/PER-12/INV-6/REN-11 audit jen pojmenoval — sjednotit je je samostatný refaktor; LAB-11 (přepočet v labu každý frame) je neměřený a levný; REN-13 (sazba písma testovatelná bez GL) by chtěla rozdělit `FontAtlas`; GEN-7 (společný strop terénu) by změnil výchozí terén i uložené světy; PLR-7 pořadí `pollEvents`/limiter a smyčka v minimalizovaném okně jsou nepotvrzená podezření závislá na platformě (trackpad opraven v 12b); PER-9 politika chybějícího `format` (tři různé) zůstává — porovnání verze už je všude stejné; fronty světla (WLD-12) se po načtení nezmenšují (~8 MB); pole vzorů receptů se nekopírují (zdokumentovaná dohoda). |
...
> **Stav (po bodech 1–12):** všechny invarianty v tabulce níž zase platí, nebo se změnilo
> jejich znění v `ARCHITECTURE.md` (dělitelnost žil GEN-9: platí pro výchozí tuning a tuner ji
> smí porušit; „~32 KB na sloupec“ upřesněno; culling/diagonály přepsáno na až 8 sekcí).
