...
`src/test/java/mc/` — **2619 kontrol**, žádný JUnit, obyčejné `main()` třídy.
...
| `FurnaceTest` | Čtyři natočení: pevné, neprůsvitné, 3,5 s, kanonicky `FURNACE`; **čelo právě na jedné straně**, vršek a bok vlastní dlaždice; jméno a zvuk; v creative jen `FURNACE`; ostatní bloky směrové boky nerozlišují; položení čelem k hráči ve všech čtyřech směrech; z natočené pece padá pec; recept z osmi kamenů (s kamenem uprostřed ne); dlaždice namalované; železný ingot. Tavení: recepty a paliva, **první ingot přesně po 10 s**, uhlí vydrží na tři kusy a dohoří, bez suroviny i s netavitelnou se palivo nezapálí, tři klacky = ingot a půl, bez ohně postup klesá, plný nebo cizí výstup pec nezapálí, **dlouhý dt netaví na dluh**. Ukládání pecí (záporné i vzdálené souřadnice), rozbitá pec vrátí obsah, nesmysly ze souboru srovnané, `SmeltBook` (vestavěnou surovinu odmítne, pec ho použije, JSON tam a zpět). Obrazovka: shift-klik rudy do suroviny a uhlí do paliva, kámen do pece nepatří, klik na výstup vezme ingot a vyprázdní ho, do výstupu se nic nepoloží, další ingot se přidá k držené hromádce, shift-klik z pece zpět do inventáře, zavření vrátí kurzor a pec si nechá sloty, **výstup craftingu dál spotřebuje suroviny**. Lab (Smelting): bez suroviny co chybí, vlastní surovina jde uložit, vestavěnou odmítne, uložená = nic neuloženého, crafting mřížka tavením nedotčená |
| `GameDirsTest` | Bez `Launch` je kořen pracovní adresář (konstanty jako dřív); složka pro Windows (i bez APPDATA), macOS a Linux (XDG i bez); přenos světů a textur včetně vnořených složek, co v cíli je, se nepřepíše, originály zůstanou, podruhé nic, do sebe sama nic |
| `LauncherTest` | Odpověď GitHubu (tag a soubory), balíček pro každý systém a ne launcher, bez balíčku null, odpověď bez tagu se odmítne; systém podle jména; verze čísly (1.10 > 1.9, v1.2 = 1.2.0, nic je nejstarší); složka verze jen z bezpečných znaků; spustitelná hra v balíčku; rozbalení vnořených složek, zipy z PowerShellu, **zip slip s `/` i `\\` odmítnut**; nainstalovaná verze (a smazaná hra = nenainstalováno) |
...
**Úprava a smazání předmětu z labu.** Když vybraná dlaždice patří předmětu z labu, tlačítko je
**Edit item**: formulář se předvyplní (`ItemDraft(ItemDef)`, hodnoty z ručně psaného souboru se
přichytí k nejbližšímu stupni, výdrž srovná hromádku na 1), **Save item** uloží pod TÝMŽ id
(`toDef` u úpravy nebere `nextId`, jméno se smí nechat, ale ne vzít jinému) a hráč nic nedostává.
...
**Hratelnost (krok 4).** Nástroj kope svůj materiál `toolSpeed`-krát rychleji
(`Items.miningSpeed`: krumpáč kámen a rudy, sekera dřevo, lopata hlínu, podle `Sound.Material`
— stejné rozdělení jako tvrdost a zvuky); cokoliv jiného v ruce kope jako ruka. Rychlost se bere
každý frame, přepnutí slotu uprostřed kopání platí hned a postup se nenuluje. Každá věc má
vlastní velikost hromádky (`ItemStack.maxCount()` → `Items.maxStack`, nástroj z labu typicky 1);
`space()` i `Container.add/room` ji respektují, prázdný slot unese do `MAX_COUNT` čehokoliv
a recept nesmí vyrobit víc kusů, než se vejde do slotu (`RecipeBook.validate`, `+` v Recipe Labu).
Uhelná ruda dává uhlí (`Mining.dropOf`), železná ruda dál rudu (pec není). Recepty jako
v Minecraftu: dvě prkna pod sebou = 4 klacky (i v malé mřížce), uhlí nad klackem = 4 pochodně,
prkno-klacek-prkno ve dvou řadách = 3 ploty; tři prkna v řadě už plot nejsou. Uhelná ruda jako
...
**`ItemRegistry` je `BlockRegistry` pro předměty.** Vestavěné v kódu: klacek (256) a uhlí (257),
dlaždice 0–7 atlasu předmětů jsou jejich rezerva. Předměty z labu v `textures/items.json` od
id 512; `nextId` jen roste, smazání id nevrátí, jeden špatný záznam neshodí ostatní, zápis
atomicky se zálohou — všechno jako u bloků. `ItemDef` nese jméno, dlaždici, velikost hromádky
a nástroj (`Tool`: pickaxe/axe/shovel podle `Sound.Material`, rychlost 1–16×); `stack`, `tool`
a `toolSpeed` smí v souboru chybět. Jméno se hlídá i proti vestavěným.

**Atlas předmětů je samostatný `textures/items.png`, se STEJNOU mřížkou jako atlas bloků**
(128 px, dlaždice 16 px, řádek 0 dole) — platí tytéž `BlockAtlas.u0/v0` a lab na něj pustí
tentýž `AtlasEditor`. Do atlasu bloků se předměty nevešly: má 64 dlaždic a lab ho ukazuje pixel
na pixel, zvětšit ho by rozbilo rozložení labu. Bez souboru procedurální klacek a uhlí;
prázdné dlaždice vestavěných se doplní, předmětů z labu dostanou šachovnici.

**Ikona předmětu je plochý obrázek ve TÉŽE dávce jako bloky.** `BlockIcon` má u vrcholu sedmý
float „zdroj" (0 = atlas bloků na jednotce 0, 1 = atlas předmětů na jednotce 1) a shader si podle
...
**Pec — hotovo (kroky 1 až 4).** Plán: (1) bloky a předměty, (2) stav pece
...
**Pec má čtyři id (21–24), protože svět nese jen bajt na buňku a pec má čelo.** Id říká, kam
čelo míří; navenek je to jeden blok `FURNACE` (čelo na +Z) — v inventáři, v creative
(`World.isVariant` natočení skryje), v receptech i po vytěžení (`World.canonical`,
`Mining.dropOf`). Natočení vzniká jen položením: `World.orientPlaced` otočí čelo proti pohledu
hráče po převažující vodorovné ose. `BlockAtlas` k tomu dostal **směrové boky** (`FACE_EAST`,
`WEST`, `SOUTH`, `NORTH`); všechny ostatní bloky je hned převedou na `FACE_SIDE`, takže se
u nich nic nemění. Směrové boky berou mesher a ikona (ikona ukazuje +Z vlevo, tedy čelo);
`BlockDraft` a lab je počítají do obsazených dlaždic, jinak by lab dal čelo pece novému bloku.
...
**Obrazovka pece je další tovární metoda `ContainerScreen.furnace`** — rozvržení Minecraftu
(surovina 56,17, palivo 56,53, výstup 116,35, batoh a hotbar jako u stolu). Sloty jsou přímo
sloty pece, takže co se do nich položí, taví se dál i po zavření. Výstup je `addOutput` s indexem
a bez craftingu: `takeResult` bere slot, na který se kliklo (dřív natvrdo index 0), a bez
crafting mřížky ho jen vyprázdní místo spotřeby surovin. Shift-klik z inventáře posílá, co se
taví, do suroviny a co hoří, do paliva (surovina má přednost — kmen jde obojím); jiné věci se
přesouvají jako obvykle. Z pece shift-klik vrací do inventáře. Šipka (plní se zleva podle
postupu) a plamínek (ubývá podle ohně) jsou obdélníky `Renderer2D` v průchodu panelu.
Pravý klik na pec ji otevře; pec bez stavu (starý soubor) dostane prázdný.

**Tavení v Recipe Labu:** přepínač **Mode: Crafting / Smelting**. Při tavení platí jen prostřední
políčko (kamkoliv se klikne do mřížky, jde tam — surovina je jedna), výsledek a počet sdílí
s craftingem; obě rozepsané práce přežijí přepnutí (surovina tavení má vlastní pole, crafting
mřížka se nemění). Info řádek říká, na co se surovina dnes taví (vestavěně / z labu); vestavěnou
surovinu lab odmítne (vestavěné tavení má přednost, recept by nikdy nevyhrál). **Save** zapíše
`textures/smelting.json` a platí hned; má-li surovina recept z labu, Clear je **Delete recipe**.
`SmeltBook` se načítá při startu po předmětech. Ověřeno GL sondou.

**Distribuce: datová složka, balení a launcher — hotovo (2026-09-25).**

- **Data hry podle systému (`GameDirs`).** Nainstalovaná hra (vstup `mc.Launch`) drží světy,
  textury z labu, zvuky a nastavení v `%APPDATA%\MinecraftClaude`, na Macu v
  `~/Library/Application Support/MinecraftClaude`, na Linuxu v `$XDG_DATA_HOME/minecraft-claude`
  (jinak `~/.local/share/...`); `-Dmc.home=…` přebije. Při prvním startu `Launch` do ní
  **zkopíruje** (ne přesune) `saves/`, `textures/`, `sounds/` a nastavení z pracovního adresáře —
  jen co tam ještě není, originály zůstanou. Cesty jsou statické konstanty (`Options.FILE`…),
  proto samostatný vstupní bod, který kořen nastaví dřív, než se ty třídy načtou.
  **`mc.Main` z IntelliJ a testy zůstávají v pracovním adresáři** — vývoj a testy tak nikdy
  nesáhnou do skutečných dat hráče.
- **Balení (`packaging/package.ps1`).** `jpackage` udělá z fat jaru aplikaci s přibalenou Javou
  (app-image, bez instalátoru a bez WiX): `dist/MinecraftClaude/MinecraftClaude.exe` a zip (~31 MB).
  Moduly: `java.desktop` (AWT pro fonty a PNG), `jdk.unsupported` (LWJGL). Verze pro jpackage
  jen z čísel (jar může být `…-shaded`).
- **GitHub Actions (`.github/workflows/release.yml`).** Tag `v*` zabalí hru i launcher na
  Windows, macOS (`-XstartOnFirstThread`) a Linuxu — každý systém na svém stroji, jinak to
  jpackage neumí — a dá je do GitHub Release. Testy běží, ale vydání neblokují (výkonové testy
  na sdíleném stroji kolísají).
- **Launcher (`LauncherMain`, `LauncherWindow`, `LauncherCore`).** Jediné, co si hráč stáhne.
  Okno ve Swingu (je v JDK, bez závislosti): Play, Check for updates, Open game folder. Zeptá se
  GitHub API na poslední release, vybere balíček pro svůj systém (ne launcher), a je-li novější
  než nainstalovaný (`versions/current.txt`, čísla po částech: v1.10 > v1.9), stáhne ho s průběhem
  a rozbalí do `<data>/versions/<tag>/`; Play pak spustí hru jako samostatný proces a zavře se.
  Bez internetu spustí poslední staženou verzi. Rozbalení odmítne „zip slip" (`../` i `..\`)
  a zná zipy z Windows PowerShellu (zpětná lomítka, složka jako `jmeno\`); macOS a Linux rozbalují
  systémovým `ditto`/`tar`, které zachová spustitelné bity. `HttpClient` se staví až při prvním
  dotazu — omezená síť nesmí shodit logiku, která síť nepotřebuje.
